...
public class ImagePagerAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        if (image.isGif()) {
            ...
            Glide.with(mContext).load(image.getPath().startsWith("http")?image.getPath():new File(image.getPath()))
                    .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                    .into(currentView);
        } else {
            Glide.with(mContext).asBitmap()
                    .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                    .load(image.getPath().startsWith("http")?image.getPath():new File(image.getPath())).into(new SimpleTarget<Bitmap>() {
                @Override
